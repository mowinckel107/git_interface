...
def git_command(command : list[str], quiet : bool = False, allowed_to_fail : bool = False) -> Git_Response:

    git_response : Git_Response = __run_git_command(command, REPO_PATH)

    if not allowed_to_fail:
        if git_response.response_text == "" or git_response.return_code != 0:
            ERROR_HANDLER(f"\n\nERROR running subprocess.run with this command:\n{" ".join(command)}\n {git_response.response_text}")
            raise BaseException("Command returned empty string while allowed_to_fail was set to False")
    ...
    print(f"command: {green_text(" ".join(command))}")
...
def get_commit_message(commit_id : str) -> str:
    ...
    response : Git_Response = git_command(["git", "show", commit_id, "-q"], quiet = True)
...
def __run_git_command( command : list[str], directory_to_call_from : Optional[StrPath] = None ) -> Git_Response:
  """Runs git command, returns output and status code."""

  return_string : str = ""
  return_code : int = -1
  try:
    response_object : subprocess.CompletedProcess[str]= subprocess.run(
        command,
        check = False,
        capture_output = True,
    ...
    return_string += response_object.stdout
    return_string += response_object.stderr

    return_code = response_object.returncode

  except:
    print("\n\nEXCEPTION caughgt in __run_git_command running subprocess.run with this command:")
    print(f"{" ".join(command)}")
    print("Calling the command from this directory:")
    print(directory_to_call_from)
    raise
    sys.exit(-1)
  finally:
    pass

  return Git_Response(return_string, return_code)
